*** This is a text (txt) file version of the Stata .do file called Nature_final with some additional information at the top. 

The text of this file can be read (by those with experience of Stata) and will explain how the various figures and tables were produced in the main paper. The .do file version can be run in Stata in combination with the .dta file to generate the tables and figures.

To run in Stata, you will need to have a working copy of Stata and install the packages estout and sutex by placing the commands below in the command line in Stata and running them each.

. ssc install sutex, replace
. ssc install estout, replace

*** Generate Tables and figures of the article: Historical Analysis of National Subjective Wellbeing using Millions of Digitized Books, published in Nature Human Behaviour  

**** Where it reads "Your Path" below, enter the path where you have the folder "TableFigure_Nature" and run this do file. All tables and figures will be saved in the folder output.
...
cd "C:\Users\Economics\Dropbox\Valence/Draft/inputs
...
graph save "C:\Users\Economics\Dropbox\Valence/Draft/inputs/britain_words.gph",  replace
